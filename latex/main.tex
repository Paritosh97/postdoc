...
\paragraph{Diffusion-based techniques} have shown more promise than other generative models such as autoregressors or GANs for 3D generation. These models work by iteratively refining a noisy input to produce a coherent output. They can be trained on large datasets of urban environments, allowing them to capture the complex relationships between different elements in the scene. %Infinicube~\cite{lu2024infinicube}... UniScene~\cite{li2025uniscene}... DynamicCity~\cite{bian2024dynamiccity}... Urban World~\cite{shang2024urbanworld}... UrbanArchitect~\cite{lu2024urban}... GraphDreamer~\cite{gao2024graphdreamer}

\paragraph{Hybrid techniques} combine procedural logic with deep generative models to achieve both control and realism. For example, ChatDyn~\cite{wei2024chatdyn} uses LLM to plan and procedurally generate the environment while uses goal-conditioned reinforcement learning, hierarchical control, and adversarial motion priors to generate dynamic aspects such as pedestrian movement. Similarly, CityDreamer4D~\cite{xie2024citydreamer} modularly integrates autoregressive token-based generation, neural rendering (e.g., NeRF-style volumetrics), and procedural traffic modeling to synthesize large-scale, time-varying 3D cities.
...
\begin{itemize}
    ...
    \item \textbf{How do we create these scenarios allowing for diverse pedestrian behaviors:} %TODO missing interactions in previous models
