...
While recent diffusion-based generative models have demonstrated strong capabilities in synthesizing urban scenes from static layouts, they remain limited in their ability to model dynamic urban scenarios. In particular, they struggle to provide precise spatial and temporal control over objects and their interactions, which is essential for simulating a realistic traffic scenario.
...
\begin{tikzpicture}[>=Stealth, node distance=0.6cm and 1cm, scale=0.7, transform shape]
  ...
  \tikzset{
    ...
    ex/.style={rectangle, draw, fill=gray!10, align=left,
      minimum width=2.5cm, minimum height=0.8cm, font=\scriptsize},
    arrow/.style={-Stealth, thick},
    panel/.style={draw,dashed, rounded corners, inner sep=0.2cm}
  }

% ----- Panel (a): Input and Splitting with MLLM labeled edges -----
\node[block, minimum height=1.2cm, text width=7cm, align=center] (input) {%
  \textbf{Scenario Prompt} \\[2pt]
};

% Four outputs below input (compact spacing)
\node[sub, below=2cm of input] (p1) {Scene Background};
\node[sub, below=1cm of p1] (p2) {2D Static Layout};
\node[sub, below=2cm of p2] (p3) {2D Dynamic Layout};
\node[sub, below=2cm of p3] (p4) {Tasks};

\node[panel, fit=(input)(p4), label=above left:(a) Prompt \& Split] {};

% Panel (b)
\node[block, right=2cm of p1] (b1) {Skybox};
\node[panel, fit=(p1)(b1), label=above right:(b) Background Generator] {};
...
\node[panel, fit=(p2)(c1)(c2)(c3), label=above right:(c) Static Scene Generator] {};


% Panel (d)
\node[block, above right=0.1cm and 1.5cm of p3] (d1) {2D Trajectories};
\node[block, right=2.5cm of d1] (d2) {Refined 2D Trajectories};
\node[block, below right=0.1cm and 1.5cm of p3] (d3) {Object States};
\node[block, right=2.5cm of d3] (d4) {Shape and Texture Keys};
\node[panel, fit=(p3)(d1)(d2)(d3)(d4), label=above left:(d) Dynamics Generator] {};



\draw[arrow] (input.south) -- node[right, font=\scriptsize, fill=white, inner sep=1pt] {MLLM} (p1.north);


\draw[arrow] (p1.east) -- node[above, font=\scriptsize, fill=white, inner sep=1pt] {360° Diffusion} (b1.west);
...
\draw[arrow] (p3.east) -- (d1.west);
\draw[arrow] (p3.east) -- (d3.west);
\draw[arrow] (d1.east) -- node[above, font=\scriptsize, fill=white, inner sep=1pt] {2D Trajectory Fit} (d2.west);
\draw[arrow] (d3.east) -- node[above, font=\scriptsize, fill=white, inner sep=1pt] {Object State VSD} (d4.west);
...
We first use a multimodal large language model (MLLM) (eg. GPT) to process the input prompt and split it into four components: scene background, static layout, dynamic layout, and tasks. The classes in the static and the dynamic layout are based on the KITTI-360~\cite{liao2022kitti360} dataset. Here is an example of how the input prompt can be split:
...
\begin{itemize}
  \item \textbf{scene\_background:} weather, time\_of\_day
  ...
  \item \textbf{static\_layout:} 
   ...
   \item \textbf{dynamic\_layout:} 
    \begin{itemize}
      ...
      \item \texttt{states}: array describing object states, including  
      \begin{itemize}
        \item \texttt{shape}  
        \item \texttt{texture\_keys}  
        \item \texttt{timing}: normalized values (0–1) matching trajectory progression  
      \end{itemize}
    \end{itemize}
...
Afternoon at a busy four-way intersection: two cars turning left, cyclist waiting. Agent must yield, stop at pedestrian light, proceed when green.
...
\begin{verbatim}
{
  ...
  "static_layout": [
    {"id": "road_1", "type": "road", "position": [0, 200], "size": [500, 100]},
    {"id": "road_2", "type": "road", "position": [200, 0], "size": [100, 500]},
    {"id": "sidewalk_1", "type": "sidewalk", "position": [0, 300], "size": [500, 20]},
    {"id": "sidewalk_2", "type": "sidewalk", "position": [300, 0], "size": [20, 500]},
    {"id": "traffic_sign_north", "type": "traffic sign", "position": [245, 180], "size": [2, 5]},
    {"id": "traffic_sign_south", "type": "traffic sign", "position": [255, 320], "size": [2, 5]},
    {"id": "traffic_sign_east", "type": "traffic sign", "position": [320, 245], "size": [5, 2]},
    {"id": "traffic_sign_west", "type": "traffic sign", "position": [180, 255], "size": [5, 2]},
    {"id": "lamp_1", "type": "lamp", "position": [230, 230], "size": [1, 5]},
    {"id": "lamp_2", "type": "lamp", "position": [270, 270], "size": [1, 5]},
    {"id": "car_01", "type": "car", "position": [110, 50], "orientation": 0, "size": [4, 2]},
    {"id": "car_02", "type": "car", "position": [390, 450], "orientation": 0, "size": [4, 2]},
    {"id": "cyclist_01", "type": "bicycle", "position": [0, 0], "orientation": 0, "size": [2, 1]},
    {"id": "pedstrian_light_1", "type": "traffic light", "position": [250, 250], "size": [1, 3]}
  ],
  "dynamic_layout": {
    "trajectories": [
      {
        "id": "car_01",
        "type": "car",
        "initial_position": [100, 210],
        "trajectory_description": [
          {"time": 0.0, "position": [100, 210]},
          {"time": 0.25, "position": [200, 210]},
          {"time": 0.5, "position": [250, 250]},
          {"time": 0.75, "position": [270, 300]},
          {"time": 1.0, "position": [300, 400]}
        ]
      },
      {
        "id": "car_02",
        "type": "car",
        "initial_position": [400, 290],
        "trajectory_description": [
          {"time": 0.0, "position": [400, 290]},
          {"time": 0.25, "position": [300, 290]},
          {"time": 0.5, "position": [250, 250]},
          {"time": 0.75, "position": [230, 200]},
          {"time": 1.0, "position": [200, 100]}
        ]
      },
      {
        "id": "cyclist_01",
        "type": "bicycle",
        "initial_position": [260, 350],
        "trajectory_description": [
          {"time": 0.0, "position": [260, 350]},
          {"time": 0.25, "position": [260, 350]},
          {"time": 0.5, "position": [260, 350]},
          {"time": 0.75, "position": [260, 320]},
          {"time": 1.0, "position": [260, 280]}
        ]
      }
    ],
    "states": [
      {
        "id": "pedstrian_light_1",
        "texture_keys": ["red", "green"],
        "timing": [0.5]
      }
    ]
  },
  "tasks": [
    "walk to the road_1",
    "stop at the pedstrian_light_1",
    "wait until pedstrian_light_1 turns green",
    "cross road_1"
  ]
}
\end{verbatim}
...
\subsection{Scene Background Generation}

A 360° diffusion model~\cite{feng2023diffusion360} can be used to generate a skybox from the scene background description provided by the MLLM. This skybox serves as the backdrop for the entire urban scenario to be used in Unity as a skybox material.

\subsection{Static Scene Generation}

The initial static layout given by the MLLM may not be perfectly aligned with realistic urban scenes. Therefore, we first use a 2D layout diffusion model trained on Kitti-360 2D layouts to obtain a more plausible configuration which matches the prior distribution. This refined 2D layout is then lifted into a 3D primitive layout. Finally, we employ a variational score distillation (VSD) approach to generate a detailed 3D scene from the 3D primitive layout as shown in~\cite{lu2024urban}.

\subsection{Dynamics Generation}

The dynamic layout consists of two components: trajectories and object states. The trajectories describe the movement of dynamic objects over time, while the object states provide information about their shape and texture at different time points.

\subsubsection{Trajectory Fitting}

Just like the static layout refining, we can use a 2D trajectory diffusion model to refine the initial trajectories provided by the MLLM. 

\subsubsection{Object State Generation using VSD}

Apart from trajectories, the dynamic layout also includes object states that describe the shape and texture of dynamic objects at different time points. For this, we need to extend the VSD approach to optimize the 3D object generation for each temporal snapshot.
