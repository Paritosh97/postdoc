...
\item $\tau: \bigcup_i \mathcal{S}_i \cup \bigcup_{i,j} \mathcal{I}_{ij} \to \mathcal{P}$ is an optional grounding map to represent the dynamic aspects w.r.t. to time.
\end{itemize}

\paragraph{Semantics}
A dynamic scenegraph $S$ encodes both (i) the \emph{static layout} through attribute maps $\mathcal{A}_i$ and relations $r_{ij}$ that are continuously active (or active on $\mathcal{T}$), and (ii) the \emph{dynamic behaviour} through temporal states $\mathcal{S}_i$ and relation active intervals $\mathcal{I}_{ij}$. The grounding $\tau$ represents the animation in 
...
% Temporal states visualization for car (moving -> stops) - moved below timeline
...
\draw[relation] (road) -- (sidewalk2) node[midway,above,attr] {next\_to};
...
% Attributes (showing sparse attribute map for some objects)
...
\node[attr, below=0.2cm of trash] {type: trash\\material: plastic};

% Temporal intervals for car states - adjusted positions
...
\node[align=left, font=\footnotesize] at (10,6) {
    $S = (V, E, \mathcal{T}, \tau)$\\
    $V = \{SW1, Road, Cross, SW2, Car, Trash\}$\\
    $E \subseteq V \times V$ with relation types\\
...
\subsection{Neural Representation of the Scene}
