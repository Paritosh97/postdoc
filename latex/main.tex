...
CLIP~\cite{radford2021learning} is a model that can be used to evaluate the quality of generated scenarios. Tested on colab with a pretrained ViT model on ImageNET.

\subsubsection{GraphDreamer}

GraphDreamer~\cite{gao2024graphdreamer} works but generation takes over 30 hours for 1 scenegraph. However, it cannot generate complex scenes with more than 3 nodes in the scene graph.
...
\textbf{How can we leverage natural language–conditioned generative models to produce urban layouts with precise spatiotemporal control over objects and their interactions?}
...
Since no existing datasets provide paired text prompts with corresponding urban layouts and tasks, we propose leveraging the capabilities of LLMs to parse and structure input prompts. Similar approaches have been explored in prior works, such as ChatDyn~\cite{wei2024chatdyn} for action planning nd LayoutGPT~\cite{feng2023layoutgpt} for layout generation.

We first process the input prompt and split it into the layout and tasks. The classes in the layout are based on the KITTI-360~\cite{liao2022kitti360} dataset. Here is an example of how the input prompt can be split:
...
\begin{itemize}
    ...
    \item \textbf{/get\_3d} – Produces 3D representations or assets.
...
This section presents a log of all experiments conducted to finetune the \textit{LLM}}. Detailed evaluations of the experiments are available \href{https://mybox.inria.fr/d/3c261c9711de473da355/}{here} for reference.

\subsection{Experiment 1}
In this experiment, the layout generation tasks is treated as a bounding box json generation task using the Qwen3-14B model~\cite{yang2025qwen3}. The expected bounding box json structure is as follows:
...
For simplicity and reducing the token size, only the static layout is considered for this experiment. In this experiment, the model is finetuned on 1/10th (~7500) of the KITTI-360~\cite{liao2022kitti360} semantic maps dataset. We use the class list and the corresponding semantic map bounding box representation pairs as the dataset for finetuning as shown below:
...
\textbf{Parameters:}
\begin{enumerate}
    \item Quantization: 4 bit
    \item Rank of LoRA: 16
    \item Alpha of LoRA: 32
    \item Learning Rate: $5\times10^{-6}$ (adaptive)
    \item Batch Size: 1 per GPU (4 GPUs used)
    \item Number of Steps/Epochs: 2
    \item Modules of the transformer finetuned: ["q\_proj", "k\_proj", "v\_proj", "o\_proj", "gate\_proj", "up\_proj", "down\_proj"]
\end{enumerate}


\textbf{Results:}
\begin{enumerate}
    \item Final train/eval loss of the LLM: 0.670500 / 0.656
    \item The generation performance of the model drops compared to the pretrained model.
\end{enumerate}


\subsection{Experiment 2}
In this experiment, the layout generation tasks is treated as a polygon json generation task using the Qwen3-14B model~\cite{yang2025qwen3}. The expected bounding box json structure is as follows:
...
For simplicity and reducing the token size, only the static layout is considered for this experiment. In this experiment, the model is finetuned on 1/10th (~7500) of the KITTI-360~\cite{liao2022kitti360} semantic maps dataset. We use the class list and the corresponding semantic map polygon representation pairs as the dataset similar to previous experiment for finetuning:
...
\textbf{Results:}
\begin{enumerate}
    \item Final train loss of the LLM: 0.637200.
    \item The generation performance of the model drops compared to the pretrained model similar to the previous experiment.
    \item The model struggles to generate complete jsons.
\end{enumerate}
...
\textbf{Results:}
\begin{enumerate}
    \item Final train loss of the LLM: 0.6066
    \item The generation performance of the model still drops compared to the pretrained model.
    \item The drop in performance is less compared to previous experiments.
\end{enumerate}
...
\textbf{Results:}
\begin{enumerate}
    \item Final train/eval loss of the LLM: 0.5486/0.5529. Loss curves shown in figure~\ref{fig:loss_curves_finetune_ep4}
    \item The model generalises better than the baseline qualitatively.
    \item Quantitatively, the CKL is still higher due to incomplete JSONs. The FID and KID are lower.
\end{enumerate}
...
\label{fig:loss_curves_finetune_ep4}
\end{figure}
...
The Fréchet Inception Distance~\cite{heusel2017gans} measures the similarity between the real and generated data distributions in the feature space of a pretrained Inception network. It models both distributions as multivariate Gaussians and computes their distance as:
\begin{equation}
\text{FID} = \|\mu_r - \mu_g\|_2^2 + \mathrm{Tr}(\Sigma_r + \Sigma_g - 2(\Sigma_r \Sigma_g)^{1/2}),
\end{equation}
...
The Kernel Inception Distance~\cite{binkowski2018demystifying} offers a similar objective to FID but estimates the squared Maximum Mean Discrepancy (MMD) using polynomial kernels. Unlike FID, KID provides an unbiased and more stable estimator for smaller sample sizes. It is defined as:
\begin{equation}
\text{KID} = \mathbb{E}_{x,x'}[k(f(x), f(x'))] + \mathbb{E}_{y,y'}[k(f(y), f(y'))] - 2\mathbb{E}_{x,y}[k(f(x), f(y))],
\end{equation}
...
\subsubsection{Classwise Kullback–Leibler Divergence (CKL)}
...
\begin{equation}
\text{CKL} = \mathbb{E}_{x \sim P_{\text{data}}} \left[ D_{\text{KL}}\big(P_{\text{data}}(y|x) \| P_{\text{gen}}(y|x) \big) \right],
\end{equation}
...
For our experiments, we use Qwen3 to generate the 3D scene layouts and tasks based on user prompts. We use the KITTI-360 dataset~\cite{liao2022kitti360} to train our multinomial diffusion model for refining the LLM generated semantic maps and trajectories.
...
\label{evaluation:qualitative:user_studies}
...
\subsubsection{04/11/2025}

\begin{itemize}

    \item \textbf{Better way to create finetune data for the VLM:}

    \item \textbf{Validation ideas:} Think of user testing, Think of metric for prompt to scene expectation, same prompt for multiple scenes, 100 scenes for user testing, think of more qualitative metrics for evaluation.

    \item \textbf{add explanations with examples to FID, KID and CKL:}

    \item \textbf{Think of the role of vlm and diffusion models}:

    \item \textbf{Check synthetic prompts }:

    \item \textbf{Fix scale of the maps to 20x20m }:

\end{itemize}

\subsubsection{24/11/2025}

\begin{itemize}

    \item \textbf{Organise the results in the doc:}

    \item \textbf{Add KID and CKL tests in the doc:}

    \item \textbf{Compare the results of previous and new methods as well as gemini:}

    \item \textbf{Add all system and user prompts to the doc:}
...
\section{Experiments}
\label{sec:experiments_diffusion}

This section presents a log of all experiments conducted to train the \textit{multinomial diffusion model}. Detailed parameters of the experiments are available \href{https://mybox.inria.fr/f/68f473ba43b348a9a2e6/?dl=1}{here} for reference. Figure~\ref{fig:whisker_plots}  shows the whisker plots of the eval losses across experiments. Table~\ref{tab:validation_results} visualises the validation results for each experiment, whereas Table~\ref{tab:llm_results} visualises the LLM conditioning results for each experiment. Figure~\ref{fig:legend} shows the legend of the generated maps.
...
% --- Define axis ---
\draw[->] (0,0) -- (9,0) node[right]{Experiment};
\draw[->] (0,0.0) -- (0,0.5) node[above]{Evaluation Loss};
...
% --- Experiment 2 ---
% Data: [0.248,0.255,0.264,0.268,0.274,0.285,0.289,0.31,0.34,0.404]
% Q1=0.268, Median=0.285, Q3=0.31, Whiskers 0.248,0.404, mean=0.294
\filldraw[fill=blue!30] (2,0.268) rectangle (3,0.31); % box Q1-Q3
\draw (2,0.285) -- (3,0.285); % median
\draw (2.5,0.31) -- (2.5,0.404); % top whisker
\draw (2.5,0.268) -- (2.5,0.248); % bottom whisker
\draw (2,0.404) -- (3,0.404); % whisker caps
\draw (2,0.248) -- (3,0.248);
\filldraw[red] (2.5,0.294) circle (0.03); % mean
\node[below] at (2.5,0) {Exp 2};

% --- Experiment 3 ---
% Data: [0.24751,0.26535,0.28919,0.3462]
% Q1=0.265, Median=0.277, Q3=0.318, Whiskers 0.247,0.346, mean=0.287
\filldraw[fill=orange!30] (4,0.265) rectangle (5,0.318);
\draw (4,0.277) -- (5,0.277);
\draw (4.5,0.318) -- (4.5,0.346);
\draw (4.5,0.265) -- (4.5,0.247);
\draw (4,0.346) -- (5,0.346);
\draw (4,0.247) -- (5,0.247);
\filldraw[red] (4.5,0.287) circle (0.03);
\node[below] at (4.5,0) {Exp 3};

% --- Experiment 4 ---
% Data: [0.2195,0.2240,0.2251,0.2540,0.3591]
% Q1=0.224, Median=0.225, Q3=0.254, Whiskers 0.219,0.359, mean=0.256
\filldraw[fill=purple!30] (6,0.224) rectangle (7,0.254);
\draw (6,0.225) -- (7,0.225);
\draw (6.5,0.254) -- (6.5,0.359);
\draw (6.5,0.224) -- (6.5,0.219);
\draw (6,0.359) -- (7,0.359);
\draw (6,0.219) -- (7,0.219);
\filldraw[red] (6.5,0.256) circle (0.03);
\node[below] at (6.5,0) {Exp 4};

\end{tikzpicture}
\caption{Whisker plot of eval losses for Experiments 2, 3 and 4}
\label{fig:whisker_plots}
\end{figure}
