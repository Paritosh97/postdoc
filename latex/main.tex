...
\begin{itemize}
    ...
    %\item concatenated instance layout and mask embeddings,
...
% Define styles
\tikzset{
  block/.style={rectangle, rounded corners, minimum width=1.8cm, minimum height=0.8cm, text centered, draw=black, fill=blue!12},
  cond/.style={rectangle, rounded corners, minimum width=1cm, minimum height=0.6cm, text centered, draw=black, fill=orange!15},
  op/.style={circle, draw=black, fill=gray!8, minimum size=6mm, inner sep=0pt},
  arr/.style={-Stealth, thick}
}

% Main flow, left to right (horizontal)
\node [block] (boxes) {Boxes Layout};
\node [block, right=0.6cm of boxes] (semmap) {Init\\Map};
\node [op, right=0.4cm of semmap] (plus) {$\oplus$};
\node [block, right=0.6cm of plus] (embed) {Embeddings};
\node [block, right=0.6cm of embed] (unet) {UNet};
\node [block, right=0.6cm of unet] (out) {Semantic\\Map};

% Conditioning nodes above and below
\node [cond, below=0.55cm of embed] (time) {Timestep $t$};

% Main arrows
\draw[arr] (boxes) -- (semmap);
\draw[arr] (semmap) -- (plus);
\draw[arr] (plus) -- (embed);
\draw[arr] (embed) -- (unet);
\draw[arr] (unet) -- (out);
...
% Label under output
\node [below=0.1cm of out] {\footnotesize Top-down categorical layout};
