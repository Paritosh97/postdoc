...
\resizebox{\textwidth}{!}{
    ...
    \end{tabular}}
\caption{Diffusion-based Models for Urban Scenario Generation}
\label{tab:diffusion_based_models}
\end{table}
...
KITTI~\cite{Geiger2012AreWR}            & 2012 & FPV        & \checkmark & \checkmark \\
Cityscapes~\cite{Cordts2016Cityscapes}      & 2016 & FPV        & \checkmark & \checkmark \\
SemanticKITTI~\cite{behley2019iccv}      & 2019 & FPV        & \checkmark & \checkmark \\
HoliCity~\cite{zhou2020holicity}          & 2020 & FPV        & \checkmark & \checkmark \\
OmniCity~\cite{li2023omnicity}          & 2023 & FPV        & \checkmark & \checkmark \\
KITTI-360~\cite{liao2022kitti360}      & 2023 & FPV        & \checkmark & \checkmark \\
GoogleEarth~\cite{googleearth}          & 2024 & FPV        & \checkmark & \checkmark \\
OSM~\cite{openstreetmap}          & 2024 & BEV        & \checkmark & \checkmark \\
CARLA~\cite{Dosovitskiy17}          & 2017 & BEV/FPV    & \checkmark & \checkmark \\
Virtual-KITTI-2~\cite{cabon2020vkitti2}      & 2020 & FPV        & \checkmark & \checkmark \\
CarlaSC~\cite{carlaSC2022}          & 2022 & BEV/FPV    & \checkmark & \checkmark \\
...
Threestudio~\cite{liu2023threestudio} is a collection of 3D generative techniques. Easy to setup and run since docker container is provided. It is well maintained by the community and has a good documentation. Thus, good to test different generative techniques for 3D models.

\subsection{City Dreamer}

CityDreamer4D~\cite{xie2025citydreamer4d} tried testing the 3D branch with static scenes but encountered issues with the container.

\subsection{Urban Architect}

Urban Architect~\cite{lu2024urban} could be useful as a starting point for generating the static urban scenario using diffusion models and score distillation sampling. For now the container has dependency issues with pytorch3d.
